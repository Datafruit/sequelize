...
class Db2QueryGenerator extends AbstractQueryGenerator {
  ...
  createTableQuery(tableName, attributes, options) {
    ...
    for (const attr in attributes) {
      const hasAttrProperty  = Object.prototype.hasOwnProperty.call(attributes, attr);
      if (hasAttrProperty) {
    ...
    for (const fkey in foreignKeys) {
      const hasKeyProperty  = Object.prototype.hasOwnProperty.call(foreignKeys, fkey);
      if (hasKeyProperty) {
  ...
  updateQuery(tableName, attrValueHash, where, options, attributes) {
    ...
    query = `UPDATE (SELECT * FROM ${this.quoteTable(tableName)} ${this.whereQuery(where, whereOptions)} FETCH FIRST ${this.escape(options.limit)} ROW ONLY) SET ${values.join(',')}`;
  ...
  deleteQuery(tableName, where, options = {}, model) {
    ...
    if (options.limit) {
      limit += ` FETCH FIRST ${  this.escape(options.limit)  } ROW ONLY`;
    }
