...
class Db2QueryGenerator extends AbstractQueryGenerator {
      ...
      _.forEach(attrValueHashes, attrValueHash => {
        tuples.push(`(${
          allAttributes.map(key =>
            this.escape(attrValueHash[key], attributes[key], { context: 'INSERT' })).join(',')})`);
      });
  ...
  updateQuery(tableName, attrValueHash, where, options, attributes) {
    ...
    query = `UPDATE (SELECT * FROM ${this.quoteTable(tableName)} ${this.whereQuery(where, whereOptions)} LIMIT ${this.escape(options.limit)} ) SET ${values.join(',')}`;
  ...
  deleteQuery(tableName, where, options = {}, model) {
    ...
    if (options.offset > 0) {
      limit = ` OFFSET ${  this.escape(options.offset)  } `;
    }
  ...
  attributeToSQL(attribute, options) {
    ...
    if (_.get(attribute, 'type.constructor.name', '') === 'JSONTYPE' ) {
      if (attribute.defaultValue) {
        template += ` DEFAULT '${JSON.stringify(attribute.defaultValue)}'`;
      }
    } else if (attribute.type !== 'TEXT' && attribute.type._binary !== true &&
  ...
  addLimitAndOffset(options) {
    ...
    if (offset > 0) {
      fragment += ` OFFSET ${  this.escape(offset)  } `;
    }
  ...
  // OR/AND/N_whereGroupBindOT grouping logic
  _whereGroupBind(key, value, options) {
    ...
    if (Array.isArray(value)) {
      ...
      nextValue = nextValue.length && nextValue.join(binding);
    } else {
      nextValue = this.whereItemsQuery(value, options, binding);
    }
    // Op.or: [] should return no data.
    // Op.not of no restriction should also return no data
    if ((key === Op.or || key === Op.not) && !nextValue) {
      const isArray = Array.isArray(value);
      // 如果内部是 json 条件，则跳过
      if (_.every(value, (item, k) => Db2QueryInterface.isJsonFilter(options.model, isArray ? item : { [k]: item }))) {
        return undefined;
      }
      return '0 = 1';
    }

    return nextValue ? `${outerBinding}(${nextValue})` : undefined;
  }

  // db2 不支持 json 筛选，忽略 json 条件
  ...
  _whereParseSingleValueObject(key, field, prop, value, options) {
    ...
    switch (prop) {
      ...
      case Op.startsWith:
        ...
        return `upper(${ options.prefix}."${key}") ${comparator} ${this.escape(`${_.toUpper(value)}%`)}`;
      case Op.endsWith:
        comparator = this.OperatorMap[Op.like];
        return `upper(${ options.prefix}."${key}") ${comparator} ${this.escape(`$${_.toUpper(value)}`)}`;
      case Op.substring:
