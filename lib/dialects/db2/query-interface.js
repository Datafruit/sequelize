 ...
 * @returns {Promise}
 */
function doClientSideFilter(queryPromise, filterFunc) {
 ...
 * 生成主键的filter
 *
 * @param {Function} queryPromise 
 * @param {Object} primaryKeyField 
 */
function getPrimaryKeyFilter(queryPromise, primaryKeyField) {
  ...
  return queryPromise.then(objOrArr => {
    if (pks.length === 1) {
      return {
        [pks[0]]: {
          $in: objOrArr.map(p => _.get(p, pks[0], ''))
        }
      };
    }
    return {
