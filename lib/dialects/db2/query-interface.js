...
const OpHelper = require('../abstract/query-generator/operators');
const AsyncStream = require('async-streamjs').default;

function wrapInArr(objOrArr) {
...
function whereItemsQuery(where, options, binding) {
  ...
  if (_.isPlainObject(where)) {
    Utils.getComplexKeys(where).forEach(prop => {
      ...
      items.push(...wrapInArr(whereItemQuery(prop, item, options)));
    });
  } else {
    items.push(...wrapInArr(whereItemQuery(undefined, where, options)));
  }
...
function whereItemQuery(key, value, options = {}) {
  ...
  if (!options.hasOr && field && !(fieldType instanceof DataTypes.JSON)) {
    return [];
  }
...
function _whereGroupBind(key, value, options) {
  ...
  if (Array.isArray(value)) {
    value = _.flatMap(value, item => {
      let filters = wrapInArr(whereItemsQuery(item, options, queryInterface.QueryGenerator.OperatorMap[Op.and]));
...
function _whereBind(binding, key, value, options) {
  ...
  return value.length
    ? /or/i.test(binding) ? _.overSome(value) : _.overEvery(value)
    : [];
}
...
function _whereJSON(key, value, options) {
  ...
  Utils.getOperators(value).forEach(op => {
    ...
    items.push(...wrapInArr(whereItemQuery(key, where, Object.assign({}, options, { json: false }))));
  });
...
function _traverseJSON(items, baseKey, prop, item, path, options) {
  ...
  const pathKey = `${baseKey}.${_.isArray(path) ? path.join('.') : path}`;

  if (_.isPlainObject(item)) {
    Utils.getOperators(item).forEach(op => {
      // const value = this._toJSONValue(item[op]);
      const value = item[op];
      items.push(...wrapInArr(whereItemQuery(pathKey, { [op]: value }, options)));
    });
  ...
  items.push(...wrapInArr(whereItemQuery(pathKey, { [Op.eq]: item }, options)));
}
...
const likeToRegexMem = _.memoize((likeExp, flags) => {
  if (10 < likeToRegexMem.cache.size) {
    likeToRegexMem.cache.clear();
  }
  const escaped = escapeForRegex(likeExp).replace(/%/g, '.*');
  return new RegExp(escaped, flags);
}, (likeExp, flags) => `${likeExp}|${flags}`);

function dataValuetoString(a) {
  if (_.isString(a)) {
    return a;
  }
  if (_.isArray(a)) {
...
const JsOpDict = {
  [Op.eq]: (a, b) => a === b,
  [Op.ne]: (a, b) => a !== b,
  [Op.gte]: (a, b) => _.isNumber(a) && a >= b,
  [Op.gt]: (a, b) => _.isNumber(a) && a> b,
  [Op.lte]: (a, b) => _.isNumber(a) && a <= b,
  [Op.lt]: (a, b) => _.isNumber(a) && a < b,
  [Op.not]: (a, b) => a !== b, // ?
  [Op.is]: (a, b) => a === b,
  [Op.in]: (a, b) => _.includes(b, a),
  [Op.notIn]: (a, b) => !_.includes(b, a),
  [Op.like]: (a, b) => {
    const val = dataValuetoString(a);
    return likeToRegexMem(b).test(val);
  },
  [Op.notLike]: (a, b) => {
    const val = dataValuetoString(a);
    return !likeToRegexMem(b).test(val);
  ...
  [Op.iLike]: (a, b) => {
    const val = dataValuetoString(a);
    return likeToRegexMem(b, 'i').test(val);
  },
  [Op.notILike]: (a, b) => {
    const val = dataValuetoString(a);
    return !likeToRegexMem(b, 'i').test(val);
...
const sqlOpToJsOpDict = Utils.getComplexKeys(OpHelper.OperatorMap).reduce((acc, symb) => {
  ...
  acc[sqlOp] = acc[sqlOp] || JsOpDict[symb];
  return acc;
}, {});
...
function _joinKeyValue(key, value, comparator, prefix) {
  ...
  return obj => {
    const colVal = _.get(obj, path, null);
 ...
 * @param {Object} where
 */
function isJsonFilter(model, where) {
  const filterKeys = getFilterAllKeys(where);
  return  _.some(filterKeys, key => {
    return model.rawAttributes[key] && model.rawAttributes[key].type instanceof DataTypes.JSON;
  });
}
 ...
 * @param {Function} jsFilterFunc
 * @param {number} resultOffset
 * @param {number} resultLimit
 * @param {number} batchSize
 */
function batchQuery(batchQueryFunc, jsFilterFunc = _.identity, resultOffset = 0, resultLimit = Infinity, batchSize = 512) {
  if (resultLimit === null || resultLimit === undefined) {
    resultLimit = Infinity;
  }
  let done = false;
  return AsyncStream.range()
    .map(batchIndex => {
      if (done) {
        return [];
      }
      return batchQueryFunc({ dbOffset: batchIndex * batchSize, dbLimit: batchSize }).then(arr => {
        if (_.size(arr) < batchSize) {
          done = true;
        }
        return arr.filter(jsFilterFunc);
      });
    })
