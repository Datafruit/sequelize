...
function whereItemQuery(key, value, options = {}) {
  ...
  if (field && !(fieldType instanceof DataTypes.JSON)) {
    return [];
  }
...
function isJsonFilter(model, where) {
  const jsonKeys = getFilterAllKeys(where).filter(key => {
...
function getFilterAllKeys(filter) {
  ...
  return _.concat(keys, _.flatten(childKeys)).filter(_.identity);
}
