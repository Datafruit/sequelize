...
class Query extends AbstractQuery {
              ...
              if (datalen > 0) {
                ...
                for (let i = 0; i < metadata.length; i++) {
                  if (!this.model) {
                    ...
                    const dataType = _.get(this.model, ['fieldRawAttributesMap', metadata[i].SQL_DESC_CONCISE_TYPE, 'type']);
                    coltypes[metadata[i].SQL_DESC_CONCISE_TYPE] = dataType.defaultType ? dataType.defaultType() : metadata[i].SQL_DESC_TYPE_NAME;
                  }
  ...
  handleInsertQuery(results, metaData) {
    if (this.instance) {
      ...
      const hasAttrProperty = Object.prototype.hasOwnProperty.call(this.model.rawAttributes, autoIncrementAttribute);
      if (hasAttrProperty &&
