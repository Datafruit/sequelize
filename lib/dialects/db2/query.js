            ...
            if (err) {
              ...
              if (datalen > 0) {
                ...
                for (let i = 0; i < metadata.length; i++) {
                  if (!this.model) {
                    coltypes[metadata[i].SQL_DESC_CONCISE_TYPE] = metadata[i].SQL_DESC_TYPE_NAME;
                  } else {
                    //处理include表数据类型
                    let dataType = null;
                    const lastSplit = metadata[i].SQL_DESC_CONCISE_TYPE.lastIndexOf('.');
                    const tablePrefix = lastSplit > 0 ? metadata[i].SQL_DESC_CONCISE_TYPE.substring(0, lastSplit) : '';
                    const fieldName = lastSplit > 0 ? metadata[i].SQL_DESC_CONCISE_TYPE.substring(lastSplit + 1) : metadata[i].SQL_DESC_CONCISE_TYPE;
                    if (tablePrefix) {
                      dataType = _.get(this, `options.includeMap.${tablePrefix}.model.tableAttributes.${fieldName}.type`);
                    } else {
                      dataType = _.get(this.model, ['fieldRawAttributesMap', fieldName, 'type']);
                    }
                    coltypes[metadata[i].SQL_DESC_CONCISE_TYPE] = dataType && dataType.defaultType ? dataType.defaultType() : metadata[i].SQL_DESC_TYPE_NAME;
                  }
