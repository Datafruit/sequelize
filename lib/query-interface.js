...
class QueryInterface {
  ...
  bulkUpdate(tableName, values, identifier, options, attributes) {
    ...
    // TODO
    if (this.sequelize.options.dialect === 'db2' && Db2QueryInterface.isJsonFilter(model, options)) {
      return this.hasJsonBulkOpertionToSelectQuer(tableName, options, model, filter => {
        const sql = this.QueryGenerator.updateQuery(tableName, values, filter, options, attributes);
        options.model = model;
        return this.sequelize.query(sql, options);
      });
    }
  ...
  bulkDelete(tableName, where, options, model) {
    ...
    if (this.sequelize.options.dialect === 'db2' && Db2QueryInterface.isJsonFilter(model, options)) {
      return this.hasJsonBulkOpertionToSelectQuer(tableName, options, model, filter => {
        return this.sequelize.query(
          this.QueryGenerator.deleteQuery(tableName, filter, options, model),
          options
        );
      });
    }
  ...
  select(model, tableName, options) {
    const options0 = Utils.cloneDeep(options);
    options0.type = QueryTypes.SELECT;
    options0.model = model;

    if (this.sequelize.options.dialect !== 'db2' || !Db2QueryInterface.isJsonFilter(model, options0)) {
      return this.sequelize.query(
        this.QueryGenerator.selectQuery(tableName, options0, model),
        options0
      );
    }
    // TODO 实现分批查询，避免一次查询过多数据
    // 如果带有 json 的筛选条件，则 findOne 不能继续使用 options.limit = 1 去查询
    delete options0.limit;
    delete options0.offset;
    delete options0.plain;

    // db2 的 json 筛选通过在客户端进行 分批查询+数组filter 实现
    // 如果带有 json 的筛选条件，则 findOne 不能继续使用 options.limit = 1 去查询，
    const options1 = Object.assign({ queryInterface: this }, options);
    const filterFunc = Db2QueryInterface.generateClientSideFilterForJsonCond(tableName, options1, model);
    const queryPromise = this.sequelize.query(
      this.QueryGenerator.selectQuery(tableName, options0, model),
      options0
    );
    return Db2QueryInterface.doClientSideFilter(queryPromise, filterFunc, options);
  }
  ...
  rawSelect(tableName, options, attributeSelector, Model) {
    ...
    if (this.sequelize.options.dialect === 'db2' && Db2QueryInterface.isJsonFilter(Model, options)) {
      const operations1 = _.cloneDeep(options);
      operations1.attributes = ['*'];
      operations1.plain = false;
      return this.hasJsonBulkOpertionToSelectQuer(tableName, operations1, Model, filter => {
  ...
  //将带有json的批量操作转成selectall生成主键filter
  hasJsonBulkOpertionToSelectQuer(tableName, options, model, operation) {
    const options1 = Object.assign({ queryInterface: this }, options);
    const filterFunc = Db2QueryInterface.generateClientSideFilterForJsonCond(tableName, options1, model);
    const options0 = Utils.cloneDeep(options);
    options0.type = QueryTypes.SELECT;
    ...
    return Db2QueryInterface.doClientSideFilter(queryPromise, filterFunc, options)
      .then(data => {
        const filter = Db2QueryInterface.getPrimaryKeyFilter(data, model.primaryKeyField);
