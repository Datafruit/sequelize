...
class QueryInterface {
  ...
  bulkUpdate(tableName, values, identifier, options, attributes) {
    ...
    if (this.sequelize.options.dialect === 'db2' && Db2QueryInterface.isJsonFilter(model, options.where)) {
  ...
  select(model, tableName, options) {
    ...
    if (this.sequelize.options.dialect === 'db2' && Db2QueryInterface.isJsonFilter(model, options0.where)) {
      ...
      return Db2QueryInterface.batchQuery(
        ({ dbOffset, dbLimit }) => {
          const options2 = _.defaults({ offset: dbOffset, limit: dbLimit, plain: false }, options0);
          return this.sequelize.query(
            this.QueryGenerator.selectQuery(tableName, options2, model),
            options2
          );
        },
        filterFunc,
        ...
        .then(res => {
          return options.plain ? res[0] || null : res;
        });
    }
